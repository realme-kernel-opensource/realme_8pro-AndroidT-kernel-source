	...
	const char *buf, size_t count)
{
	...
	if (!cmd_bufs) {
		cmd_bufs = kzalloc(SZ_4K, GFP_KERNEL);
	...
	const char *buf, size_t count)
{
	...
	if ((oppo_get_panel_brightness() == 0) && onscreenfp_status) {
		pr_err("notify fingerpress return as screen is off\n");
		return count;
	}
	if (!display->panel->oppo_priv.prj_flag) {
	...
	int power_mode, void *disp)
{
	...
	int blank;
	#ifdef OPLUS_FEATURE_AOD_RAMLESS
	static int old_power_mode = SDE_MODE_DPMS_OFF;
	#endif /*OPLUS_FEATURE_AOD_RAMLESS*/
	...
	case SDE_MODE_DPMS_ON:
		...
		if(OPPO_DISPLAY_AOD_SCENE == get_oppo_display_scene()) {
			if (sde_connector_get_fp_mode(connector)) {
				#ifdef OPLUS_FEATURE_AOD_RAMLESS
				if (display->panel->oppo_priv.is_aod_ramless &&
						((old_power_mode == SDE_MODE_DPMS_LP1) || (old_power_mode == SDE_MODE_DPMS_LP2))) {
		...
		if (display->panel->oppo_priv.lcd_cabc_support) {
			oplus_display_panel_set_cabc(&g_cabc_mode);
			pr_info("[%s:%d] SDE_MODE_DPMS_ON CABC recover,cabc:%d\n", __func__, __LINE__, g_cabc_mode);
		}
...
#endif /* OPLUS_FEATURE_AOD_RAMLESS */
/* fp type config */
static DEVICE_ATTR(fp_type, S_IRUGO|S_IWUSR, oppo_ofp_get_fp_type_attr, oppo_ofp_set_fp_type_attr);
